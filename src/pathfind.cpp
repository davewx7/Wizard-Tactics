...
bool path_cost_calculator::legal_move_endpoint(const location& a) const
{
	return true;
}
...
namespace {
void find_possible_moves_internal(int max_cost, const path_cost_calculator& calc, route_map& routes)
{
	bool found_locs = false;
	hex::location adj[6];
	for(route_map::const_iterator i = routes.begin(); i != routes.end(); ++i) {
		get_adjacent_tiles(i->first, adj);
		for(int n = 0; n != 6; ++n) {
			if(!calc.allowed_to_move(adj[n])) {
				continue;
			}
			const int cost = i->second.cost + calc.movement_cost(i->first, adj[n]);
			if(cost > max_cost) {
				continue;
			}
			route_map::const_iterator current = routes.find(adj[n]);
			if(current != routes.end() && current->second.cost <= cost) {
				continue;
			}

			found_locs = true;

			route& r = routes[adj[n]];
			r.steps = i->second.steps;
			r.steps.push_back(adj[n]);
			r.cost = cost;
		}
	}

	if(found_locs) {
		find_possible_moves_internal(max_cost, calc, routes);
	}
}
}
...
void find_possible_moves(const hex::location& loc, int max_cost, const path_cost_calculator& calc, route_map& routes)
{
	...
	route& r = routes[loc];
	r.cost = 0;
	r.steps.push_back(loc);
	find_possible_moves_internal(max_cost, calc, routes);

	route_map::iterator itor = routes.begin();
	while(itor != routes.end()) {
		itor->second.cost /= 10;
		if(calc.legal_move_endpoint(itor->first) == false) {
			routes.erase(itor++);
		} else {
			++itor;
		}
