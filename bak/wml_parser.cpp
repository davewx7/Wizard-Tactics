...
node_ptr parse_wml_internal(const std::string& error_context, const std::string& doc, bool must_have_doc)
{
	...
	while(i != doc.end()) {
		if(util::isnewline(*i)) {
			...
			if(name == "import" || name == "include") {
				...
				wml::node_ptr node(parse_wml_from_file("data/" + filename, name == "import"));
				if(node) {
...
{
	return parse_wml_internal("", doc, must_have_doc);
}

node_ptr parse_wml_from_file(const std::string& fname, bool must_have_doc)
{
	try{
		return parse_wml_internal(fname, preprocess(sys::read_file(fname)), must_have_doc);
